...
export async function listUserImages(userID: string, params: ListImagesQuery): Promise<ListResponse> {
...
export async function adminDeleteImage(id: string): Promise<void> {
  await apiFetch(`/api/admin/images/${encodeURIComponent(id)}`, { method: 'DELETE' })
}
