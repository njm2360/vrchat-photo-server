...
export async function impersonateUser(
  ...
  const res = await apiFetch(`/api/admin/users/${id}/impersonate`, {
    method: "POST",
  });
  return res.json();
}
...
export async function deleteProxyCache(url: string): Promise<void> {
  ...
  await apiFetch(`/api/admin/proxy/cache?${q.toString()}`, {
    method: "DELETE",
  });
}
...
export async function checkProxyHost(
  url: string,
): Promise<{ allowed: boolean; matched_pattern: string | null }> {
  const q = new URLSearchParams({ url });
