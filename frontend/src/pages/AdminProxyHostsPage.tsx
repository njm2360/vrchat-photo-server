  ...
  return (
    <div className="mt-4 flex items-center justify-center gap-1">
      <button
        ...
        className={`${btnCls} border-[#444] text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0] disabled:cursor-not-allowed disabled:opacity-40`}
      >
        前へ
      </button>
      {pages.map((p) => (
...
export default function AdminProxyHostsPage() {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setAddError(
        err instanceof ApiError ? err.message || "登録に失敗しました" : "登録に失敗しました",
      );
    } finally {
      setAddLoading(false);
    }
    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <input
          ...
          placeholder="パターンで検索..."
          className="min-w-48 flex-1 rounded border border-[#444] bg-[#2a2a2a] px-3 py-1 text-sm text-[#e0e0e0] placeholder-[#555] focus:border-[#666] focus:outline-none"
        />
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className="cursor-pointer rounded border border-[#0d6efd] px-3 py-1 text-xs text-[#0d6efd] hover:bg-[#0d6efd]/10"
          >
            + 追加
          </button>
        </div>
      ...
      <div className="mb-4 flex items-center gap-2 rounded border border-[#2a2a2a] bg-[#1a1a1a] p-3">
        <span className="text-xs whitespace-nowrap text-[#666]">URL チェック</span>
        <input
          type="text"
          value={checkUrl}
          onChange={(e) => {
          ...
          className="flex-1 rounded border border-[#444] bg-[#2a2a2a] px-3 py-1 font-mono text-sm text-[#e0e0e0] placeholder-[#555] focus:border-[#666] focus:outline-none"
        />
        <button
          onClick={() => void handleCheck()}
          disabled={checkLoading || !checkUrl.trim()}
          className={`${btnCls} whitespace-nowrap disabled:cursor-not-allowed disabled:opacity-50`}
        >
          {checkLoading ? "確認中..." : "確認"}
        </button>
        {checkResult &&
          (checkResult.allowed ? (
            ...
            <>
              <span className="text-xs whitespace-nowrap text-red-400">✗ 拒否</span>
              <button
                onClick={() => {
                  const host = (() => {
                    try {
                ...
                className="cursor-pointer rounded border border-[#0d6efd] px-2 py-0.5 text-xs whitespace-nowrap text-[#0d6efd] hover:bg-[#0d6efd]/10"
              >
                このホストを許可する
              </button>
            </>
      ...
      {loading && items.length === 0 ? (
        ...
        <p className="text-sm text-[#666]">許可ホストがありません</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm text-[#ccc]">
            <thead>
              <tr className="border-b border-[#333] text-xs text-[#888]">
                <th className="py-2 pr-3 text-left font-normal">パターン</th>
                <th className="py-2 pr-3 text-left font-normal">メモ</th>
                <th className="py-2 pr-3 text-left font-normal">状態</th>
                <th className="py-2 pr-3 text-left font-normal whitespace-nowrap">
                  追加日時 / 追加者
                </th>
                <th className="py-2 pr-3 text-left font-normal whitespace-nowrap">
                  更新日時 / 更新者
                ...
                <tr key={item.id} className="border-b border-[#2a2a2a] hover:bg-[#1a1a1a]">
                    ...
                    {editingNoteId === item.id ? (
                      ...
                      >
                        <input
                          ...
                          className="min-w-0 flex-1 rounded border border-[#555] bg-[#2a2a2a] px-2 py-0.5 text-xs text-[#e0e0e0] focus:border-[#0d6efd] focus:outline-none"
                          onKeyDown={(e) => e.key === "Escape" && setEditingNoteId(null)}
                        />
                        <button
                          type="submit"
                          disabled={savingNoteId === item.id}
                          className="cursor-pointer rounded border border-[#0d6efd] px-1.5 py-0.5 text-xs text-[#0d6efd] hover:bg-[#0d6efd]/10 disabled:opacity-50"
                        >
                        ...
                        <button
                          ...
                          className="cursor-pointer rounded border border-[#444] px-1.5 py-0.5 text-xs text-[#888] hover:text-[#ccc]"
                        >
                          ✕
                        </button>
                      </form>
                      ...
                      <button
                        ...
                        className="group w-full cursor-pointer truncate text-left text-xs text-[#888] hover:text-[#ccc]"
                        title="クリックして編集"
                      >
                        {item.note ? (
                  ...
                  <td className="py-2 pr-3">
                    <button
                      ...
                      className={`cursor-pointer rounded border px-2 py-0.5 text-xs transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${
                        item.enabled
                          ? "border-green-700 text-green-400 hover:border-red-700 hover:bg-red-900/20 hover:text-red-400"
                          : "border-[#555] text-[#666] hover:border-green-700 hover:bg-green-900/20 hover:text-green-400"
                      }`}
                  ...
                  <td className="py-2 text-right">
                    <button
                      ...
                      className="cursor-pointer rounded border border-red-800 px-2 py-1 text-xs text-red-400 hover:bg-red-900/30 hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-50"
                    >
      ...
      {showAdd && (
        ...
        >
          ...
          >
            <h2 className="mb-1 text-base font-semibold text-[#e0e0e0]">許可ホストを追加</h2>
            ...
            >
              <div className="flex flex-col gap-1">
                <label className="text-xs text-[#aaa]">
                  パターン <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  value={addPattern}
                  ...
                  required
                  autoFocus
                  className="w-full rounded border border-[#444] bg-[#2a2a2a] px-3 py-2 font-mono text-sm text-[#e0e0e0] focus:border-[#0d6efd] focus:outline-none"
                />
              </div>
              <div className="flex flex-col gap-1">
