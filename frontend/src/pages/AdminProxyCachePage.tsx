...
function SortBtn({
  ...
  return (
    <button
      onClick={() => onSort(col)}
      className="ml-1 cursor-pointer text-[#666] hover:text-[#aaa]"
    >
      {active ? (order === "asc" ? "▲" : "▼") : "⇅"}
    </button>
  );
    ...
    <div className="mt-4 flex items-center justify-center gap-1">
      ...
      {pages.map((p) => (
        <button
          ...
          className={`${btnCls} ${p === currentPage ? "border-[#0d6efd] bg-[#0d6efd]/10 text-[#0d6efd]" : "border-[#444] text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"}`}
        >
          {p + 1}
        </button>
      ))}
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      {loading && filtered.length === 0 ? (
        ...
        <p className="text-sm text-[#666]">キャッシュがありません</p>
      ) : (
          ...
          <table className="w-full border-collapse text-sm text-[#ccc]">
            <thead>
              <tr className="border-b border-[#333] text-xs text-[#888]">
                ...
                <th className="py-2 pr-4 text-left font-normal">状態</th>
                <th className="py-2" />
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={item.url} className="border-b border-[#2a2a2a] hover:bg-[#1a1a1a]">
                    <td className="max-w-xs py-2 pr-4">
                      <span
                        className="block truncate font-mono text-xs text-[#aaa]"
                        title={item.url}
                      >
                        {item.url}
                      </span>
                    </td>
                    <td className="py-2 pr-4 text-xs whitespace-nowrap text-[#888]">
                    ...
                    <td className="py-2 pr-4">
                      {expired ? (
                        <span className="rounded bg-[#333] px-1.5 py-0.5 text-xs text-[#888]">
                          期限切れ
                        </span>
                      ) : (
