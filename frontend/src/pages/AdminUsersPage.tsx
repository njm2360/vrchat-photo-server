  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        ...
        autoFocus={autoFocus}
        className={`w-full rounded border bg-[#2a2a2a] px-3 py-1.5 text-sm text-[#e0e0e0] focus:outline-none ${
...
function SortBtn({
  ...
  return (
    <button
      onClick={() => onSort(col)}
      className={`flex cursor-pointer items-center gap-0.5 font-medium whitespace-nowrap transition-colors ${active ? "text-[#e0e0e0]" : "hover:text-[#e0e0e0]"}`}
    >
      {label}
...
export default function AdminUsersPage() {
  ...
  function handleSort(col: string) {
    if (col === sort) setOrder((prev) => (prev === "asc" ? "desc" : "asc"));
      ...
      setSort(col as NonNullable<ListUsersQuery["sort"]>);
      setOrder("desc");
    }
  }
  ...
  const btnCls =
    "text-sm px-3 py-1.5 rounded border border-[#444] text-[#aaa] hover:text-[#e0e0e0] hover:bg-[#2a2a2a] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer";
  const pgBtnCls =
    "px-2 py-1 text-xs rounded border border-[#444] text-[#aaa] hover:text-[#e0e0e0] hover:bg-[#2a2a2a] disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer";
    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="cursor-pointer rounded bg-[#0d6efd] px-3 py-1.5 text-sm text-white hover:bg-blue-600"
          >
            + 新規ユーザー作成
          </button>
        </div>
        ...
        <div className="flex gap-1">
          {(["", "active", "disabled"] as const).map((f) => (
            <button
              ...
              className={`cursor-pointer rounded border px-2 py-1 text-xs transition-colors ${statusFilter === f ? "border-[#0d6efd] bg-[#0d6efd]/10 text-[#0d6efd]" : "border-[#444] text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"}`}
            >
        ...
        <div className="flex gap-1">
          {(["", "admin", "user"] as const).map((f) => (
            <button
              ...
              className={`cursor-pointer rounded border px-2 py-1 text-xs transition-colors ${roleFilter === f ? "border-[#0d6efd] bg-[#0d6efd]/10 text-[#0d6efd]" : "border-[#444] text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"}`}
            >
      ...
      <div className="overflow-x-auto rounded-lg border border-[#333]">
        <table className="w-full min-w-max text-sm">
          <thead>
            <tr className="bg-[#1e1e1e] text-left text-[#aaa]">
          ...
          <tbody>
            {users.map((user, i) => (
              ...
              >
                <td className="px-4 py-2.5 font-medium text-[#e0e0e0]">
                  {user.username}
                ...
                <td className="px-4 py-2.5">
                  {user.is_admin ? (
                    <span className="rounded border border-blue-700 bg-blue-900/40 px-1.5 py-0.5 text-xs text-blue-400">
                      管理者
                    </span>
                  ) : (
                ...
                <td className="px-4 py-2.5">
                  {user.is_disabled ? (
                    <span className="rounded border border-red-800 bg-red-900/40 px-1.5 py-0.5 text-xs text-red-400">
                      無効
                    </span>
                  ) : (
                    <span className="rounded border border-green-800 bg-green-900/30 px-1.5 py-0.5 text-xs text-green-500">
                      有効
                    </span>
                  )}
                </td>
                <td className="px-4 py-2.5 text-xs text-[#888]">{formatDate(user.created_at)}</td>
                <td className="px-4 py-2.5 text-xs text-[#888]">
                  {formatDate(user.last_login_at)}
                </td>
                <td className="px-4 py-2.5 text-right text-[#aaa]">{user.image_count}</td>
                ...
                <td className="px-4 py-2.5">
                  <button
                    ...
                    className="cursor-pointer rounded border border-[#444] px-2 py-0.5 text-sm leading-tight text-[#aaa] hover:border-[#666] hover:text-[#e0e0e0]"
                    title="操作"
                  >
                    ···
                  </button>
      ...
      {showCreate && (
        ...
        >
          <div
            className="w-96 rounded-lg border border-[#333] bg-[#1e1e1e] p-6 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="mb-4 font-medium text-[#e0e0e0]">新規ユーザー作成</h2>
            <div className="flex flex-col gap-3">
              <UsernameInput
                value={createUsername.value}
                onChange={createUsername.onChange}
                checkState={createUsername.state}
                autoFocus
              />
              <div className="flex flex-col gap-1">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded border border-[#444] bg-[#2a2a2a] px-3 py-1.5 pr-14 text-sm text-[#e0e0e0] focus:border-[#0d6efd] focus:outline-none"
                  />
                  <button
                    type="button"
                    onClick={() => setShowNewPw((v) => !v)}
                    className="absolute top-1/2 right-2 -translate-y-1/2 text-xs text-[#888] hover:text-[#aaa]"
                  >
                    {showNewPw ? "非表示" : "表示"}
                  </button>
                </div>
              ...
              <div className="mt-1 flex justify-end gap-2">
                <button
                  onClick={closeCreate}
                  className="cursor-pointer rounded border border-[#444] px-3 py-1.5 text-sm text-[#aaa] hover:text-[#e0e0e0]"
                >
                  キャンセル
                </button>
                <button
      ...
      {resetTarget && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="mb-4 font-medium text-[#e0e0e0]">パスワードリセット</h2>
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1">
      ...
      {actionMenuUser && (
        <div className="fixed inset-0 z-50" onClick={() => setActionMenuUser(null)}>
          <div
            className="absolute w-48 rounded-lg border border-[#333] bg-[#1e1e1e] py-1 shadow-xl"
          ...
          >
            <button
              onClick={() => {
                setActionMenuUser(null);
                void navigate(`/admin/users/${actionMenuUser.id}/images`);
              }}
              className="w-full cursor-pointer px-4 py-2 text-left text-sm text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"
            >
              画像一覧
            </button>
            <button
              onClick={() => {
                setActionMenuUser(null);
                setRenameTarget(actionMenuUser);
                renameUsername.reset(actionMenuUser.username);
                setRenameError("");
              }}
              className="w-full cursor-pointer px-4 py-2 text-left text-sm text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"
            >
            ...
            {actionMenuUser.username !== selfUsername && (
              <>
                <button
                  onClick={() => {
                    setActionMenuUser(null);
                    void handleImpersonate(actionMenuUser);
                  }}
                  className="w-full cursor-pointer px-4 py-2 text-left text-sm text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"
                >
                  なりすましログイン
                ...
                <button
                  onClick={() => {
                    setActionMenuUser(null);
                    setResetTarget(actionMenuUser.id);
                    setResetPw("");
                    setResetError("");
                    setShowResetPw(false);
                  }}
                ...
                <button
                  onClick={() => {
                    setActionMenuUser(null);
                    void handleToggleAdmin(actionMenuUser);
                  }}
                  disabled={togglingId === actionMenuUser.id}
                  className={`w-full cursor-pointer px-4 py-2 text-left text-sm disabled:cursor-not-allowed disabled:opacity-50 ${actionMenuUser.is_admin ? "text-orange-400 hover:bg-orange-900/20" : "text-[#aaa] hover:bg-[#2a2a2a] hover:text-[#e0e0e0]"}`}
                >
                  {togglingId === actionMenuUser.id
                    ? "変更中..."
                ...
                <button
                  onClick={() => {
                    setActionMenuUser(null);
                    void handleToggleDisabled(actionMenuUser);
                  }}
                  disabled={disablingId === actionMenuUser.id}
                  className={`w-full cursor-pointer px-4 py-2 text-left text-sm disabled:cursor-not-allowed disabled:opacity-50 ${actionMenuUser.is_disabled ? "text-green-500 hover:bg-green-900/20" : "text-yellow-500 hover:bg-yellow-900/20"}`}
                >
                  {disablingId === actionMenuUser.id
                    ? "変更中..."
                ...
                <button
                  onClick={() => {
                    setActionMenuUser(null);
                    void handleDelete(actionMenuUser);
                  }}
                  disabled={deletingId === actionMenuUser.id}
                  className="w-full cursor-pointer px-4 py-2 text-left text-sm text-red-500 hover:bg-red-900/20 disabled:cursor-not-allowed disabled:opacity-50"
                >
            ...
            <button
              onClick={() => {
                setActionMenuUser(null);
                void handleRevokeSessions(actionMenuUser);
              }}
              className="w-full cursor-pointer px-4 py-2 text-left text-sm text-red-500 hover:bg-red-900/20"
            >
              セッションを全て無効化
            </button>
            <button
              onClick={() => {
                setActionMenuUser(null);
                void handleClearImages(actionMenuUser);
              }}
      ...
      {renameTarget && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="mb-1 font-medium text-[#e0e0e0]">ユーザー名変更</h2>
            <p className="mb-4 text-xs text-[#666]">現在: {renameTarget.username}</p>
            <div className="flex flex-col gap-3">
              <UsernameInput
                value={renameUsername.value}
                onChange={renameUsername.onChange}
                checkState={renameUsername.state}
                autoFocus
              />
